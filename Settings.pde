  ...
  Settings(String _s) {
    try {
      ...
      for (int i=0;i<data.length;i++) {
        if (data[i].equals("Minimum Words")) minWords = setInt(data[i+1]);
        if (data[i].equals("Maximum Words")) maxWords = setInt(data[i+1]);
        if (data[i].equals("Minimum Parts Per Word")) minSyllables = setInt(data[i+1]);
        if (data[i].equals("Maximum Parts Per Word")) maxSyllables = setInt(data[i+1]);
        if (data[i].equals("CamelCase")) camelCase = setBoolean(data[i+1]);
      }
    } catch(Exception e) {
